#################################################################################################################################################################
# Remarks:     Script to export data from Log Analytics to Blob Storage. Added suport for blob tags on 06/24/21.
#              
#
             ...
             foreach ($row in $output) 
                ...
                else
                {
                    ...
                    if($filedisksize.Length+$resultsize -gt $global:maxfilesize){
                        ...
                        saveToFile -output "," -filepath "$($folder_for_output)\$($getperiod)_$($filecount).json"
                        saveToFile -output $body_join -filepath "$($folder_for_output)\$($getperiod)_$($filecount).json"
                        "Saving to file..." | Out-File "$($location)explogging_\log.txt" -Append
...
function GetUserInputObj 
{
    ...
    $startperiod+" "+$endperiod | Out-File "$($location)explogging_\log.txt" -Append
...
function UploadToBlob
{
       PARAM( 
        ...
        
        {
            ...
            "Error during AzCopy $status" | Out-File "$($location)explogging_\log.txt" -Append
        ...
        IF ($status -contains "Number of Transfers Failed: 0"){
            ...
            "Error : $_.ErrorDetails.Message" | Out-File "$($location)explogging_\log.txt" -Append
            return $false
        }
...
"Completed queries:$iloop" | Out-File "$($location)explogging_\log.txt" -Append
